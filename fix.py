...
def generate_pdf_report(assessment_data):
    try:
        ...
        filename   = "ThreatReport_" + timestamp + ".pdf"
        ...
        title_s   = S("T2",  fontSize=18, alignment=1,
                      fontName="Helvetica-Bold", spaceAfter=4,
                      textColor=colors.HexColor("#1a1a2e"))
        sub_s     = S("Su2", fontSize=9, alignment=1,
                      textColor=colors.grey, spaceAfter=8)
        heading_s = S("H2",  fontSize=12, fontName="Helvetica-Bold",
                      spaceBefore=12, spaceAfter=4,
                      textColor=colors.HexColor("#1a1a2e"))
        body_s    = S("B2",  fontSize=9, leading=14, spaceAfter=4,
                      textColor=colors.HexColor("#333333"))
        note_s    = S("N2",  fontSize=8, leading=12, spaceAfter=4,
        ...
        story += [
            ...
            Paragraph("Dark Web Intel Tool | BCA Final Year Project | OSINT | SDG 16", sub_s),
        ...
        story += [mt, SP(0.2)]
        story += [Paragraph("1. EXECUTIVE SUMMARY", heading_s), HR(), SP(0.08),
                  Paragraph(summary, body_s), SP(0.1)]
        ...
        if breaches:
            story.append(Paragraph("Found in " + str(len(breaches)) + " breach record(s).", body_s))
            b_rows = [["#","Source","Date","Data Exposed","Affected","Severity"]]
            for i, b in enumerate(breaches, 1):
                b_rows.append([str(i), str(b.get("source",""))[:28],
                    str(b.get("breach_date","")), str(b.get("data_type",""))[:30],
                    str(int(b.get("affected_count",0))), str(b.get("severity",""))])
            bt = Table(b_rows, colWidths=[0.3*inch,1.9*inch,0.9*inch,1.7*inch,1.0*inch,0.8*inch])
            ...
            story.append(Paragraph("No matches found.", body_s))
        story.append(SP(0.1))
        story += [Paragraph("4. DARK WEB INTELLIGENCE (SIMULATED)", heading_s), HR(), SP(0.08),
                  Paragraph("Dark web scan detected " + str(dw.get("dark_web_hits",0)) + " source(s).", body_s), SP(0.06)]
        if dw_matches:
            dw_rows = [["Source","Type","Title","Level","Verified"]]
            for m in dw_matches:
                dw_rows.append([str(m.get("sim_source",""))[:28], str(m.get("leak_type","")),
                    str(m.get("post_title",""))[:30], str(m.get("threat_level","")),
            ...
            story.append(dwt)
        story += [SP(0.06), Paragraph("NOTE: Simulated data. No TOR access.", note_s), SP(0.15)]
        story += [Paragraph("5. SECURITY RECOMMENDATIONS", heading_s), HR(), SP(0.08)]
        for i, rec in enumerate(recos, 1):
            story.append(Paragraph(str(i) + ". " + rec, body_s))
        story += [SP(0.2), HR(), SP(0.08)]
        story.append(Paragraph(
            "DISCLAIMER: BCA Final Year Academic Project, Sushant University. "
            "Simulated data only. No illegal systems accessed. SDG 16.", note_s))
        doc.build(story)
        print("[OK] PDF generated: " + filename)
        return filename
    except Exception as e:
        import traceback
...
with open("modules/alert_system.py", "w", encoding="utf-8") as f:
    f.write(alert)

print("alert_system.py rewritten successfully")
print("Line 11 is:", alert.split("\\n")[10])